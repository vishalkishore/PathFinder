...
void handle_post(http_request request) {
  // Create a shared pointer to store request
  auto request_ptr = std::make_shared<http_request>(std::move(request));

  request_ptr->extract_json()
      .then([](json::value body) {
        ...
        try {
          ...
          error_response["error"] = json::value::string(e.what());
          return error_response;
        }
      })
      .then([request_ptr](pplx::task<json::value> previous_task) {
        try {
          json::value response = previous_task.get();
          request_ptr->reply(status_codes::OK, response);
        } catch (const std::exception &e) {
          json::value error_response = json::value::object();
          error_response["error"] = json::value::string(e.what());
          request_ptr->reply(status_codes::InternalError, error_response);
        }
      });
}
